...
`problem-index.json` (formerly listed here as `cache.json`, unspecified) is an array of lightweight per-problem summaries — `{ folderId, frontendId, title, slug, difficulty, topics, language, acceptedAt }` — accumulated across every accepted submission, regardless of platform. It exists so `leetcode/README.md` (see section 14a) can be regenerated on every commit without re-fetching or re-parsing every problem folder's own `metadata.json` from GitHub.
...
# 14a. leetcode/README.md — Multi-Way Problem Index

LeetCode-specific (other platforms would get their own platform-scoped index, not a shared one). Regenerated from `.internal/problem-index.json` on every accepted submission so it scales as the number of solved problems grows, rather than staying a flat unsorted list. Generated by `generateLeetcodeIndexReadme` (`packages/generators`).

Sections, in order:

1. **Summary line** — total solved + Easy/Medium/Hard breakdown.
2. **Contents** — anchor links to every section below.
3. **By Problem Number** — the basic index: every problem, ascending, with difficulty and topics.
4. **By Difficulty** — three subsections (Easy/Medium/Hard).
5. **By Topic** — one collapsible `<details>` section per topic tag, alphabetical. A problem with multiple topics is listed under every one of them (topics are not mutually exclusive), which is the categorical index.
6. **By Language** — one collapsible `<details>` section per submitted language.
7. **Recently Solved** — the most recent 15 problems, newest first.

Collapsible `<details>` blocks (native GitHub markdown rendering) keep large topic/language sections scannable instead of one unbroken page.
